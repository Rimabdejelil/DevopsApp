...
def kyc():
    ...
    #api_key = request.form.get('api_key')  # Récupérez la clé API à partir des paramètres de requête

    #if not api_key or not is_valid_api_key(api_key):
     #   return jsonify({"error": "Clé API non valide"})
    ...
    # Assurez-vous que les noms des champs dans votre formulaire correspondent aux noms des images envoyées
        ...
        #recto_image = request.files['recto_image']
                ...
                #recto_image.save(os.path.join('static', recto)) # Utilisation d'un horodatage pour le nom du fichier
                recto_image.save(recto)

                #recto_copy = f"recto_copy_{int(time.time())}.jpg"
                #recto_image.save(os.path.join('static', recto_copy))
        ...
        if ((recto_image.filename) and (not verso_image.filename)):
            try:
        #recto_image = request.files['recto_image']
                recto = f"static/recto.jpg"  # Utilisation d'un horodatage pour le nom du fichier
                recto_image.save(recto)
                #o = get_orientation(recto)
                #if o == 3 :
                 #   detected_orientation = np.ascontiguousarray(np.rot90(cv2.imread(recto), 1))
    ...
    # Utilisez ThreadPoolExecutor pour traiter les images en parallèle
        ...
        # Utilisez submit pour soumettre les tâches de traitement des images
                ...
                if not isinstance(verso_future.result(), tuple) and isinstance(recto_future.result(), tuple) :
                    ...
                    #for key, value in recto_results.items():
                     #   if value is not None:  # Vérifiez si la valeur n'est pas nulle dans recto_results
                      #      response_data[key] = value
                              # Mettre à jour uniquement si la valeur de verso_results est None
        ...
        # Obtenez les résultats après le traitement en parallèle
                    #if verso_future.result() == "<Response 43 bytes [200 OK]>" :
                    #    print("ok")
                     #   
